  ...
  return (
    <main className="relative min-h-screen overflow-hidden px-4 py-6 jarq-text sm:px-6 lg:px-8">
      ...
      <section className="mx-auto max-w-6xl">
        <header className="mb-6 flex flex-col gap-4 border-b pb-5 md:flex-row md:items-end md:justify-between jarq-border">
          <div>
          ...
          <div className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold jarq-glass">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {courses.map((course, index) => (
            <Link
              ...
              className="button-lift group rounded-2xl p-5 transition hover:-translate-y-1 hover:border-cyan-300/60 hover:bg-white/15 jarq-glass"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-md bg-cyan-300/20 text-cyan-100">
                <BookOpen size={22} />
              </div>
              <div className="mt-5 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] jarq-muted">
                Уровень {levelLabel(course.level)} · Мир {index + 1}
              </div>
              <h2 className="mt-2 text-xl font-semibold">{course.title}</h2>
              <p className="mt-2 min-h-12 text-sm leading-6 jarq-muted">
